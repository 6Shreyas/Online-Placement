	...
	public ResponseEntity<?> updateStatus(@RequestBody UpdateStatusDTO dto) {
		...
		if(dto.getStatus().equals("Approved")) {
			...
			email.sendMail(job.getStudent().getEmail(), subject, 
					"Dear "+job.getStudent().getSname()+",<br>You have been shortlisted for the interview for "+job.getCompany().getCname()+".<br>"
					+"Any further details regarding the selection process will be communicated through the mail itself.<br> Regards.<br>"+job.getCompany().getCname() );
		}
